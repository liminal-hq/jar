...
module's _implementation language and process boundary_ — Rust, running
...
(PWA) _and_ Tauri desktop, so it genuinely needs both adapters. Jar
...
(current move-to target) were _client-side, per-frame_ state even in the
...
Everything in `SPEC.md` §6's persistence list _except_ window geometry
...
would: _"Prevents the 'spiral of death' at very high speed multipliers —
the sim runs at most this many ticks per frame, gracefully slowing
apparent speed rather than starving the renderer."_ Jar's slider goes to
...
each version bump explaining _why_ the layout changed. Copy that
...
closed is not simulated" already accepts _some_ looseness around exactly
...
Using it keeps `jar-core` scoped to _simulation_ facts only, not general
...
reads parent A's _current_ `RigidBody` position (which it already has)
...
- No resolution of mobile sync — `SPEC.md` §3's Android companion already
  ...
  doc.
- Window position/size/always-on-top persistence is explicitly _not_
