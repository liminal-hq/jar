...
The tank's one shadow-casting light (`directionalLight`, `TankScene.tsx`) sets `gl.shadowMap.autoUpdate = false` plus a single one-shot `needsUpdate = true` right after `onCreated`, rather than recomputing its shadow depth pass every frame (issue #94). This is only correct because _nothing shadow-relevant in the scene moves_: fish (`FishModel.tsx`'s body/dorsal/gill/tail meshes, and `fishGeometry.ts`'s `wrapInPivot` for the pectorals/mouth) and swaying plant blades (`Plants.tsx`) — the only things in the tank that translate or deform per frame — all have `castShadow` removed, leaving only the static castle/decor/sand floor casting. `receiveShadow` stays everywhere it was, including on fish and plants, so they still visibly pick up the (now-static) castle/decor shadow — only _casting_ a shadow themselves is gone. If a future addition makes something else in the tank move (a new decor animation, a second light), it needs to either avoid `castShadow` too or this whole freeze needs revisiting — a moving caster under a frozen shadow map reads as its shadow silently detaching and staying behind.
