...
- **Tail** (`tailPivotRef.rotation.y`): `Math.sin(phase) * amplitude`, where `amplitude` widens with `turnRate` (capped) when actively swimming, or drops to a low `0.08` idle flutter at `speed < 0.05` — the idle-flutter rule from the original spec survives unchanged; a motionless fish still reads as dead, not asleep. Fan/Forked tails get their own `RIGID_TAIL_AMPLITUDE_MULTIPLIER` (`1.8×`) on top of this base amplitude: a single rigid pivot at the plain base swing reads as stiff, since (unlike Veil, below) it has no secondary bend to sell the motion instead. `phase` is accumulated frame to frame (`tailPhase.ts`'s `advanceTailPhase`, `phase += frequency * delta`, wrapped modulo 2π) rather than derived from absolute clock time — `frequency` itself varies every frame (speed, turn-rate damping, chase overdrive all feed it), and deriving phase as `elapsedTime * frequency` let the instantaneous beat rate drift from the intended `frequency` by an amount proportional to session length. This same accumulated `phase` also gates `thrustEnvelope.ts`'s physics impulse (§4.1), so the tail's visible beat and the body's actual propulsion pulse are the same signal by construction.
- **Veil tail only** (`veilGeometry`'s vertex positions, `fishGeometry.ts`): a genuine per-vertex bend on top of the pivot rotation above — each vertex's local `x` (distance from the tail tip) sets how much extra angle it picks up, phase-lagged by `1.1` rad per the original spec's stagger constant, so the tail reads as one continuous wave rather than a rigid paddle. Fan/Forked stay rigid single-pivot (their own amplitude multiplier above compensates instead); only Veil's shape calls for the extra motion.
- **Pectoral fins** (`pectoralPivotRef`/`pectoralFarPivotRef.rotation.z`): a smaller, faster flutter, driven by its own accumulated phase (advanced at `1.3×` the tail's `frequency`, the same `advanceTailPhase` accumulator pattern as the tail's own `phase`) rather than derived as `phase * 1.3` — once `phase` wraps at 2π, multiplying it by a non-integer factor breaks continuity right at the wrap, so the flutter needs its own independent accumulator to stay smooth.
...
- **Floor/decor:** a hand-authored castle (`Castle.tsx`) with a real cut-through doorway, built with the same SVG-extrusion technique `FishModel.tsx` uses (`decor-svg/castle.svg`), flatter and wider than a realistic castle so the door can actually clear a fish's own collider (see `decorLayout.ts`'s header comment); five swaying plant clusters (`Plants.tsx`, up from the original plan's three), built the same way, that visibly bend away from a fish swimming close by (`plantSway.ts`'s `plantDisturbanceAngle`); a two-tone sand floor (`SandFloor.tsx`) with a real SVG overlay, one larger rock, and scattered pebbles, replacing the original flat placeholder box. Plant sway: same sine-based technique as fish spine/gecko gait, driven by a per-vertex bend — same visual effect as the CSS `sway` keyframe it replaces, built with a genuinely different (3D-native) mechanism, not adapted from it.
