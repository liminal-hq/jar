...
- **Manual pilot (debug tool):** one fish at a time can be handed to the keyboard instead of its own AI, armed from a per-row control in the Fish monitor window (`windows/FishMonitor/FishMonitorWindow.tsx`) rather than a product screen. `ManualPilotBehaviour` (`render/steering/manualPilotBehaviour.ts`) is inserted into every fish's steering list in `Wander`'s own slot — right after `TankContainmentBehaviour`/`CastleAvoidanceBehaviour`, before `Wander`/`Separation`/`Arrive`/the chase pursuit behaviour. Idle it contributes nothing; engaged, its force is sized to exactly `vehicle.maxForce`, which — through the same insertion-order budget described above, no mode-system changes needed — silences every behaviour after it while leaving the two avoidance behaviours' first claim on the budget untouched, so a piloted fish still can't be forced through the glass or into the castle. Built to force the exact square-on wall-approach geometry a containment-force stall needs to reproduce on demand, and kept afterward as a small permanent tool in its own right.
