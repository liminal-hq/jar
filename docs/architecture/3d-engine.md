...
- **Helpers:** `@react-three/drei` (loaders, `useGLTF`, `Sparkles` as a fallback particle primitive), `@react-three/rapier` (physics, §5), `@react-three/postprocessing` (the tank's CRT/neon frame effect and the Fish eye window's lens-distortion effect, §10.3).
...
- On window resize, scale the tank volume (not the camera) to fit the new aspect ratio within the frame bezel's interior — this preserves `SPEC.md`'s "percentage-based" resizing intent (§2) directly: a critter at `x=50` is always centred regardless of window size, exactly as it is in the 2D prototype's `left: c.x + '%'`.
...
| Steering (how does it get there)               | `YUKA.Vehicle` position/velocity, active behaviours                                          | `/src/render/steering`              |
...
- **Active behaviours: `WanderBehavior` + `SeparationBehavior` + two anticipatory-avoidance behaviours, `TankContainmentBehaviour` (the glass) and `CastleAvoidanceBehaviour` (the castle's collision boxes, `decorLayout.ts`'s `CASTLE_COLLIDER_BOXES`).** Deliberately _not_ `CohesionBehavior` or `AlignmentBehavior` — Jar's critters are named, individual pets, not an anonymous school; boid flocking makes multiple named fish move in lockstep, which reads as less alive, not more. `TankContainmentBehaviour`/`CastleAvoidanceBehaviour` are added first — Yuka's `SteeringManager` accumulates each behaviour's force in insertion order and stops once `vehicle.maxForce` is reached, so adding either last could let `Wander`/`Separation` exhaust the whole budget before either avoidance behaviour ever contributed — and neither is mode-gated: both stay at a fixed weight of 1 in every motion mode, including `paused`/`settled`, since even a resting fish shouldn't be able to drift into the glass or the castle. Each fish's own margin sizes both, but — since fish are genuinely flat, not spherical (`fishCollider.ts`'s `ColliderHalfExtents`, `[thickness, height, length]`) — that margin is computed live every frame from the fish's own _current_ size and heading rather than fixed at construction: `worldExtentX`/`worldExtentZ` project the fish's live half-extents onto whichever world axis matters at its current yaw (fish only ever rotate freely in yaw; pitch is small and clamped, roll is always 0), so a nose-on approach gets the full length margin (as safe as a fixed length-based margin ever was) while a broadside one gets close to just the thickness margin — letting a fish approach, and pass through, a gap only as wide as its real body, with no gap-specific exemption geometry anywhere. Using the fish's live current size rather than its fixed eventual adult one is deliberate too: since the margin is already recomputed every frame, there's no staleness risk in it, and a still-growing fry gets a proportionally smaller, honest margin from birth instead of the adult-sized one it would've needed under the old fixed-at-construction design. This is also what retired `CastleAvoidanceBehaviour`'s old doorway-only exemption: a nose-on fish's live margin at the doorway (thickness plus a small buffer) never reaches the doorway's flanking walls from its own centreline, so the symmetric-cancellation stall that exemption existed to route around can no longer happen for a fish actually threading the doorway nose-on — the feature this margin exists for. It's a narrower guarantee than "for every orientation," though: a fish sitting broadside-ish (very roughly 40°-140° off nose-on, worst for the widest fin types) exactly on the doorway centreline can still see the flanking walls' pushes cancel — the same class of accepted, self-resolving edge case (`WanderBehavior`'s own continuous heading jitter breaks the symmetry in practice) the old exemption's own comment already accepted for its one non-fitting combination, just spanning a wider yaw range now rather than one fixed case. `Wander`/`Separation` (and, mode-dependent, `Arrive`) instead fade their own `.weight` toward a per-mode target over time rather than snapping on/off — an instant behaviour-set flip was a real, fixed stutter bug (a full-strength `ArriveBehavior` force landing in one frame at every night settle/wake transition).
  - _Stretch, explicitly opt-in later:_ a per-relationship "bonded pair" attraction (e.g. parent/child, or two critters flagged as attached) as a small custom steering force scoped to that one relationship — not a global flock behaviour. Not required for v1.
...
- **Arrival at favourite spot:** ~30% of the time a fish's current wander/target cycle resolves, redirect it to `fav.{x,y,z}` using an `ArriveBehavior` (slowing radius) rather than snapping — reproducing the same 30%-of-the-time rule `Jar.dc.html`'s mock uses, re-expressed as a proper steering behaviour instead of a linear-interpolation target.
...
- **Pause behaviour:** 1–4 s pause at each stop, same range `SPEC.md` §5 already specifies.
...
- **Superseded plan, not yet built:** this section is still the intended design (`SPEC.md` §5's floor/branch climb rule), but it's unbuilt — only the fish are rendered so far (§7's own note). §4.3's `crawlSurfaces.ts`, built for the snail (issue #98), is a more general analytic version of the same "surfaces + folds between them" idea (a `NavMesh` proper vs. a small closed set of flat, hand-authored faces) — worth considering as the gecko's own eventual floor↔branch mechanism instead of a separate Yuka `NavMesh`, rather than building two independent surface-traversal systems. Not decided or scoped here; gecko rendering stays out of scope for issue #98.

### 4.3 Snail (crawl surfaces)

- **Shared analytic surface model:** `render/environment/crawlSurfaces.ts` — a critter-agnostic (no `Species` import, no three.js import), pure-math module built for the snail but written so a future second crawling critter (see the gecko note above) could reuse it verbatim. A `CrawlFace` is a flat rectangle in world space (origin + two in-plane axes + an outward normal); a `FaceLink` records a real, physically-shared edge between two faces' own actual world-space endpoints — not a symbolic name — so `advance()` can re-parameterize a crossing pose continuously into the neighbour regardless of how the two faces' axes happen to be oriented relative to each other. A `CrawlPose` (face id + in-plane `u`/`v` + heading) is the crawler's whole placement.
- **Face inventory, v1-scoped:** the sand floor; the four glass walls, from the floor up to a capped height below the rim's light-strip band; and, for each of `decorLayout.ts`'s five `CASTLE_COLLIDER_BOXES`, that box's top face plus its four vertical sides. Deliberately built from the avoidance-tuned _collider_ boxes, not the visible castle silhouette — those already differ by up to ~0.22 world units (`decorLayout.ts`'s own header comment), so a crawling snail's feet can sit slightly proud of or sunk into the visible geometry in places. Accepted for v1. Links: floor↔each wall's bottom edge, wall↔wall at the four vertical corners, floor↔each castle side's base (except the lintel, which floats above the doorway and never touches the floor), and each castle side↔its own top; adjacent castle sides are not linked to each other (a crawler reaching a side face's left/right edge bounces rather than wrapping the box) — out of scope for v1.
- **API:** `advance(pose, distance)` marches along the current heading, folding across any linked edge crossed and clamping+reflecting (a bounce) at any unlinked one, in up to `MAX_FOLD_STEPS` folds per call. `turn(pose, deltaAngle)` rotates in place. `poseToWorld(pose)` returns a world position plus an orthonormal frame (`forward`/`up`/`right`, as plain vectors — the snail controller, §4.4, builds a `THREE.Quaternion` from `forward`/`up` itself, keeping this module three.js-free). `randomFloorPose(random)` and `dropToFloor(worldPoint)` give a spawn/landing position clear of the castle, reusing the same `keepClearOfCastle` margin logic `Fish.tsx` already uses for a favourite-spot roll.

### 4.4 Snail controller & physics

- **`SnailController`, not a `SteeringSystem` participant (settled):** `render/tank/Snail.tsx` is self-contained — no Yuka vehicle, no `useSteeringRegistry` entry. It owns one `CrawlPose` directly and advances it itself in `useFrame`, rather than joining the fish's shared steering/registry machinery. Accepted cost: fish don't separation-steer around a snail the way they do around each other; the snail's own collider (below) still keeps a fish from physically overlapping one.
- **Activity/tuck state machine:** `render/tank/snailBehaviour.ts` — a pure, unit-tested step function (`motionState.ts`'s "states as a closed union" discipline, but with real per-mode timing logic, since a fish's equivalent lives inline in `Fish.tsx` and a snail's is complex enough to want testing on its own). States: `crawling | pausing | settling | sealed | waking | startled | detached`. Sleep/wake is driven every frame by `domain/dayNight.ts`'s `isAsleep('Snail', override, simNight)` — nocturnal, so `'day'` seals it, `'night'` wakes it, `'active'` never seals it. `settling`/`waking` are the ramp phases that drive `tuckProgress` (fed straight into `SnailModel`'s matching prop) between 0 (emerged) and 1 (sealed); `sealed` holds at 1. `startled` runs the identical ramp in `SnailModel`'s `'startle'` tuck mode, which `snailGeometry.ts`'s `tuckPoseForMode` already caps below the operculum-seal window, so a startled snail flinches but never seals — triggered by a click on the model (`Snail.tsx`'s own `onClick`, not a separate click-detection layer).
- **Movement:** while `crawling`, `Snail.tsx` advances its `CrawlPose` via `crawlSurfaces.advance()` at a slow, deliberately exaggerated-for-legibility speed (project convention — bias toward visual exaggeration over strict physical accuracy, the same philosophy behind the fish's own speed/burst tuning in §4.1), steering its heading with a small continuous random-walk bias rather than Yuka wander, so it reads as ambling rather than beelining. `pausing` (a periodic roll, much slower cadence than the fish's 8-15s one) holds still without changing tuck state, mirroring the fish's own `paused` mode. `poseToWorld()`'s `forward`/`up` vectors become a `THREE.Quaternion` built so the RigidBody's local `+Z`/`+Y` map to world `forward`/`up` — the same axis convention `fishCollider.ts` uses, so the model group needs the identical fixed `-90°` yaw correction `Fish.tsx` applies (both models are authored facing `+X`). Fully script-authored orientation, no gravity fight — the same discipline as the fish's dynamic body, just on a **kinematic** (`type="kinematicPosition"`) `RigidBody` instead, moved via `setNextKinematicTranslation`/`setNextKinematicRotation` every frame.
- **Collider:** `render/tank/snailCollider.ts` mirrors `fishCollider.ts`'s discipline exactly (a pure module, half-extents hard-derived from `snailGeometry.ts`'s measured constants, one policy headroom constant for the eyestalks, `lifeStageScale` applied live) with one structural addition: a `centreOffsetY`, since the snail model's local origin isn't the box's centre the way the fish's roughly-centred body is — the foot's sole sits well below origin, the shell apex well above. `halfLength` comes from the foot's tail-tip extent (farther from origin than the toe, so it alone contains the whole foot); `halfHeight` from half the shell's sole-to-apex distance (Turret is genuinely the tallest); `halfThickness` from `max(shell depth, foot depth)/2` plus the eyestalk headroom constant (Ramshorn's thin 14-unit shell keeps it the thinnest box, even though its `foot`-depth floor matches every other shell's). `centreOffsetY` places the box so its bottom face lands exactly on the sole/contact line `crawlSurfaces.ts` also uses.
- **Dawn-on-a-wall detach:** sleep beginning while the snail's current `CrawlPose.faceId` isn't `'floor'` transitions `snailBehaviour.ts` straight to `detached` rather than `settling` — `Snail.tsx` eases the RigidBody from wherever it was down to a `crawlSurfaces.dropToFloor()`-computed landing point over `DETACH_SINK_DURATION_SEC`, a smoothstep-eased straight-line descent with a small decaying horizontal sway layered on top (a presentation choice — scripted, not real physics/gravity). Once landed, the pose snaps to the computed floor pose and the state machine proceeds into `settling` (if still asleep) or `crawling`.
- **Fish-knocks-a-snail-loose (first use of this pattern in the codebase):** both `Fish.tsx`'s and `Snail.tsx`'s `RigidBody`s carry a `userData={{ kind, critterId }}` tag (`'fish'`/`'snail'`) — the first `userData` use and the first `onCollisionEnter` wiring anywhere in this codebase. The snail's `onCollisionEnter` checks `event.other.rigidBody?.userData?.kind === 'fish'` and, unless already `detached`, feeds a one-shot `fishContact` signal into `snailBehaviour.ts`, which transitions into the identical `detached` sequence as the dawn case above. No velocity/force threshold for v1 — any contact triggers it, a deliberate "playful framing rather than pre-engineering a physics threshold" choice; a minimum relative velocity or a short per-snail cooldown is the natural first addition if live testing shows a fish casually brushing past a snail knocking it down too often.
...
| `sex` (male/female)      | Not modelled in the 2D prototype; now tracked in the sim (`SPEC.md` §5) and shown in the critter card (`SCREENS.md` W2), but never had a _visual_ form | Subtle dimorphism per §6.7 — tail-scale and saturation multipliers, not a second mesh |
...
- **Shared rotation pivot**: the tail's geometry is hinge-shifted (`extrudeAtHinge`) into a local space whose origin is the tail pivot, not the body's own shared origin — the tail material's own baked-in `spaceOffsetX` (`swimWaveShader.ts`'s port of `applySwimWave`'s old `rotationOffsetX` parameter) shifts a tail vertex into that shared body space before rotating and back out after, so the body and tail rotate about the same world pivot and their physical seam stays closed as amplitude changes.
- **Normals** (GPU, `swimWaveShader.ts`, issue #94): corrected per vertex via the inverse-transpose of the wave's local (x,z) Jacobian — the analytically-correct generalization of "rotate the rest normal by the vertex's own angle," which undercorrects wherever the angle changes fastest (high-`tipGain` tail tips) because it ignores how that angle varies with x. This replaced a CPU-side `computeVertexNormals()` recompute from the deformed positions every frame — a discrete, face-averaged approximation to the same continuous quantity — run alongside a full position rewrite (see the port note below).
...
- Not built: baked `AnimationMixer` clips from a generator's auto-rig/auto-animate output — moot with no rigged asset pipeline at all (§6.1).
- **GPU port** (`swimWaveShader.ts`, issue #94): the wave's displacement/normal math above now runs in a vertex shader — injected into each of the body/dorsal/tail `MeshStandardMaterial`s via `onBeforeCompile` — rather than a CPU loop (`swimWave.ts`'s `applySwimWave`) rewriting `position`/recomputing normals and flagging the whole `BufferGeometry` for a full GPU re-upload, 3x per fish, every frame. Per-fish-part constants (`onsetX`/`tipCommonX`/`seamU`/`tipGain`/`lag`/`spaceOffsetX` — all fixed at spawn) are baked directly into each material's shader source; only `phase`/`amplitude`/`bend` are real uniforms, written once per frame. `applySwimWave`/`buildWaveTables` (`swimWave.ts`) are no longer called from the render path but are kept, still covered by `swimWave.test.ts`, as the tested reference the shader's formula is checked against. One accepted trade-off: `geometry.attributes.position` now stays at the fish's rest pose permanently (nothing rewrites it), so R3F's pointer-event raycasting (`Fish.tsx`'s click-to-select) and frustum culling see the rest pose, not the currently-bent one — a minor hit-testing inaccuracy for a small silhouette's modest bend, not a functional loss, and it drops the CPU-side rest-position snapshots and the per-frame `computeBoundingSphere()` the old code needed (`applySwimWave`'s own former doc comment explained why).
- **Shadow tracking** (`swimWaveShader.ts`'s `attachSwimWaveDepthMaterial`): because `position` is never rewritten (previous bullet), three.js's shadow-map pass would otherwise substitute its own generic depth material for these meshes — it only forwards a handful of texture/clipping properties, never `onBeforeCompile` — and render each fish's raw _rest-pose_ geometry into the shadow map regardless of how the visible material bends it, permanently detaching a swimming fish's shadow from its actual shape. Each of the body/dorsal/tail meshes (`FishModel.tsx`) is given a `customDepthMaterial` — a `MeshDepthMaterial` with the _same_ position-only displacement injected (no normal correction; a shadow depth pass never samples normals, and `meshdepth`'s own `#include <beginnormal_vertex>` isn't even compiled in without a displacement map) — built from the _same_ `SwimWaveFrameUniforms` object as its visible material, so the one `setSwimWaveUniforms` call each frame keeps both in sync automatically.
...
- Explicitly **not** doing: a separate body shape, a separate rig, or any behavioural difference — `sex` affects breeding eligibility (`SPEC.md` §5) and this one visual pass, nothing else.
...
Both run continuously whenever their toggle is on — including at night; `SPEC.md` doesn't call for them to pause, and there's no behavioural reason they should (they're ambient environment, not creature behaviour governed by the sleep state in §4.1).
...
The **Fish eye** debug window (§4.1) carries a second, unrelated `@react-three/postprocessing` pass of its own — always on, not frame-gated, since a fisheye lens is that window's whole identity rather than one of several selectable frame looks. Built from stock effects: `LensDistortionEffect` (from the underlying `postprocessing` package, wrapped via `@react-three/postprocessing`'s `wrapEffect` since it has no dedicated JSX component) for the barrel warp, plus `Vignette` and a radially-modulated `ChromaticAberration` for edge darkening and fringing. See `render/effects/FishEyeLensEffect.tsx` and `render/effects/fishEyeLens.ts`.
...
- No buoyancy force as the _primary_ locomotion driver (steering is; buoyancy is idle-only flavour, §5.3).
