...
- **Phase lag** (`SWIM_WAVE_LAG`): a vertex's angle lags the shared `phase` by an amount proportional to how far along the span it sits, so the bend reads as a wave traveling from body to tail tip rather than the whole fish flexing in lockstep.
