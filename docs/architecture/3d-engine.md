...
- **Light toggle:** a real hood-mounted LED strip fixture (`LedLightStrip.tsx`), not just a cosmetic sprite — the LEDs themselves are sealed inside an opaque housing (a bare exposed strip reads as blinding, not as tank lighting) with only a soft diffuser panel visible on its underside, and a `rectAreaLight` mounted flush with that panel genuinely lights the scene below it, in addition to the pre-existing soft additive plane near the tank's ceiling. Both toggle together on the `light` boolean from the drawer. The castle also carries its own always-on ground-level uplighting fixture prop (`Castle.tsx`'s `GroundUplight`) — a small lamp at its base shining up the wall, independent of the drawer's light toggle.
