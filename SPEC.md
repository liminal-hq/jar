...
- **Genetics**: hue = parents' mean ± 18° (originals roll fresh across the full wheel); tail type (fan / forked / veil) from one parent (fish only); shell type (coil / ramshorn / turret) and pattern (solid / banded / spotted) each from one parent (snail only — fish/gecko have neither); spots inherit with 70% chance if either parent has them (fish/gecko only); personality inherits from one parent 70% of the time (fresh roll from the full pool otherwise). Sex (male/female) is rolled 50/50 independently at birth — not inherited from either parent. Originals roll fresh.
- **Stats** (per critter):
...
- **Movement**: fish pick random targets (bold ones roam the full height), sleepy fish pause between trips; geckos crawl the floor, climb to the branch ~35% of the time, and pause 1–4 s at each stop. (Movement/steering implementation: `docs/architecture/3d-engine.md` §4.)
