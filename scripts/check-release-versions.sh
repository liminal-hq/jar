...
# Only the files that represent the shipped app's own identity -- the
# workspace/desktop package.json, tauri.conf.json, the jar binary crate's own
# Cargo manifest, and its native plugin adapter. crates/jar-core and
# crates/jar-protocol are deliberately excluded: pure internal libraries
# (per CLAUDE.md's "no I/O" / "shared types" split), never independently
# distributed, with no reason to track the app's own release version.
FILE_SPECS=(
    "Workspace package.json|package.json|json"
    "Desktop package.json|apps/jar/package.json|json"
    "Tauri config|apps/jar/src-tauri/tauri.conf.json|json"
    "Desktop Cargo manifest|apps/jar/src-tauri/Cargo.toml|toml"
    "Plugin Cargo manifest|plugins/tauri-plugin-jar/Cargo.toml|toml"
)
