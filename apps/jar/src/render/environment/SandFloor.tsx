...
export function SandFloor() {
  return (
    <group>
      <mesh position={[0, FLOOR_TOP_Y - 0.1, 0]}>
        <boxGeometry args={[TANK_WIDTH, 0.2, TANK_DEPTH]} />
        <meshStandardMaterial color={SAND_BASE_COLOUR} roughness={0.9} />
      </mesh>

      {/* Overlay + pebbles: authored top-down, so rotate the extruded slab
          flat (its own "depth" axis becomes world Y) and lift it just
