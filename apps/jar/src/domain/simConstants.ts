// Constants mirrored from `crates/jar-core/src/tick.rs` for frontend-only
// presentation concerns (life-stage render scale) that don't need a round
// trip through the Rust core — `age_sec` itself is already pushed on every
// `TickUpdate`, so this is a pure function of already-synced data, not a
// second independent fact that could disagree with the core. Keep the
// thresholds in sync by hand; these are small, stable numbers from
// SPEC.md §5, not part of the generated protocol.
//
