...
An agent driving the running app (screenshots, DOM snapshots, JS eval, simulated input) uses the `tauri-mcp-server` MCP tool, which talks over a WebSocket to `tauri-plugin-mcp-bridge` — a debug-only plugin (`#[cfg(debug_assertions)]` in `apps/jar/src-tauri/src/lib.rs`) that only exists in debug builds, never release ones.

- **Starting the app for automation: use `bun run tauri:dev` (or `bun run --cwd apps/jar tauri:dev`), not plain `bun run tauri dev`.** `tauri:dev` applies `apps/jar/src-tauri/tauri.conf.dev.json` on top of the base config, which sets `"withGlobalTauri": true` — the one thing that config adds. The app itself never needs that (it imports the Tauri JS API as ES modules), but the MCP bridge's non-macOS `execute_js` path phones its result back specifically through the global `window.__TAURI__.core.invoke`/`.event.emit`. Without `withGlobalTauri`, that global is never injected: the injected script runs, but has no way to call back, so the Rust side always hits its own internal timeout. Plain `tauri dev` still starts the bridge fine (`[MCP][PLUGIN][INFO] MCP Bridge plugin initialized` on port 9223) and non-JS-eval calls (`ipc_get_backend_state`, `manage_window`) work — it's specifically `webview_execute_js`, screenshots, DOM snapshots, `read_logs` console source, and `webview_wait_for` that silently hang and time out at whatever timeout was requested, with no error on the app side to point at the cause.
- **If it's still broken after starting with the right script:** the long-lived Node `mcp-server-tauri` helper process can independently leak stale WebSocket connections to port 9223 across a session that spans many `driver_session start`/`stop` cycles (e.g. resumed across `/compact`s or days) — `ss -tnp | grep 9223` showing more than one `ESTAB` connection from the same `node` PID confirms it. `driver_session stop`/`start` does not clean this up. The only fix found so far is killing that Node process outright (`ps aux | grep mcp-server-tauri`; its parent is `npm exec @hypothesi/tauri-mcp-server`) so a fresh one spawns — it does not auto-respawn mid-session, so the MCP tool goes fully unavailable until reconnected (fresh Claude Code session, or whatever mechanism registered it). Check the config file first, though — that's the more common and more fixable cause.
