...
- Its own window (nothing covers the tank). Shows every option at once:
  ...
  - Tank toggles: Light (plus a colour chip picker while it's on — Daylight, Warm, Moonlight, Reef, Jungle, Sunset, Party), a Castle light intensity slider (0-200%, always shown — the castle's own ground fixture stays on regardless of the Light toggle above), Bubbles/Mist (plus a Bubble intensity slider, 0-200%, while Bubbles is on in Fish mode), Critter sounds (fish/gecko only — snails are silent)
  - Simulation speed: slider 1–60× with a **Real time** chip (snaps to 1×). Hint line explains the current jar-day length.
  - Jar clock line, `+ Add a critter` — one button per species the current habitat holds (`+ Add a fish` and `+ Add a snail` in the aquarium; just `+ Add a gecko` in the terrarium)
  - **Restore default settings** — resets every option above (habitat, frame, theme + variant, light + colour + castle intensity, bubbles/mist + intensity, sound, simulation speed) plus always-on-top, back to its documented default in one step; also resets the tank menu's Day/night override to Always active, since that's a frontend-only preference this command's backend settings reset never touches. Applies immediately, no confirmation
