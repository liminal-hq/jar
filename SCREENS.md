...
- Click the tank → the **drawer** slides out from behind it to one side (right by default; flips to the side with screen room). It overlaps the bezel by ~18 px so it reads as attached. Clicking the tank again, or leaving the drawer idle, closes it.
